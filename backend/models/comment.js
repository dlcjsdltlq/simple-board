...
export const addComment = (articleId, commentIndex, userId, userName, text, depth, targetIndex) => {
    const commentInstance = new CommentModel({
        articleId, commentIndex, userId, userName, text, depth, targetIndex, date: new Date()
    });
...
export const getComments = (articleId, start, end) => {
    return CommentModel.find({ articleId, commentIndex: { $gte: start, $lte: end }}).select('articleId commentIndex userId userName text depth targetIndex date -_id').sort('commentIndex');
};
